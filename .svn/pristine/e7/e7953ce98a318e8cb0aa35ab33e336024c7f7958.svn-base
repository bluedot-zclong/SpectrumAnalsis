							...
							<tr>
							  ...
							  <td class="active text-center">
							   ...
							   <td class="warning text-center">
							    <form role="form"  id="form1" runat="server" action="${path}/recyclebin/update.do" method="post">
							   	<#if item.algorithm_name??>
							   		<input type="hidden" name="alid" value="${item.algorithm_id}"/>						   		
							   	</#if>
								<#if item.spectrum_name??>
									<input type="hidden" name="speid" value="${item.spectruminfo_id}"/>
								</#if>
									<input type="hidden" name="state" value="0"/>
							   		<button type="submit" class="btn btn-info update" data-toggle="modal" data-target=".bs-example-modal-sm">恢复</button>
							   		</form>
							   		 <form role="form"  id="form1" runat="server" action="${path}/recyclebin/update.do" method="post">
							   	<#if item.algorithm_name??>
							   		<input type="hidden" name="alid" value="${item.algorithm_id}"/>						   		
								...
								<input type="hidden" name="state" value="2"/>
							   		 <button type="submit" class="btn btn-info delete" data-toggle="modal" data-target=".modeldelete">删除</button>
