...
//	}
	 ...
	 * @return
	 */
	public static boolean loginVerify(String email, String password, String vcode, String validateCode, boolean devMode) {
		
		if (StringUtils.isAnyBlank(email, password)) {
			return false;
		}else if(!isEmail(email)) {
			return false;
		}else if (!devMode && !VerifyCodeUtils.verifyCode(vcode, validateCode)) {
			// 如果dev 为true 直接跳过验证码
